...
This file documents our process of downloading and processing pollen records from the [Neotoma Paleoecology Database](https://www.neotomadb.org).To extend this approach to other taxa, regions, and times, fork the [Range Mapper Github repository](https://github.com/NeotomaDB/RangeMapper) and modify the code, as directed below.

Check out [Range Mapper](http://open.neotomadb.org/RangeMapper/index.html), the visualizations mapping plant taxon range shifts from the Last Glacial Maximum to present in North America, Europe, and Australasia. The current interactive maps are only a few of many that could be possible with code that links Neotoma"s deep paleoecological data repository to CARTO"s CartoVL JavaScript library.
...
setLocation <- function(){
  ...
  all_bbox <- list(c(-169, 24, -52, 75), c(-11, 35, 47, 72), c(105, -51, 179, 0))
  
  # Set coordinates 
  location <- list(geoJSON = all_geoJSON[[i]],
          bbox = all_bbox[[i]])
  location$sf <- geojsonsf::geojson_sf(location$geoJSON)[[i]]
...
# Approximately US box for debugging
location <- list(geoJSON = '{"type": "Polygon",
        "coordinates": [[
            [-119, 24],
                [-119, 45],
...
# Removed site 13923 (dataset 21422) b/c "The dataset %d has multiple default chronologies. Chronology %d has been used.",  : invalid format '%d'; use format %s for character objects BUT it seemed to have 1 default chron?
# Removed site 27896
# removed_downloads <- all_downloads[c(846,1045)]
# all_downloads <- all_downloads[-846]
# all_downloads <- all_downloads[-1045]
# c(51727,51729,51741,51743, 51745, 51747)
# all_downloads[[846]]$collunits <- all_downloads[[846]]$collunits[2]
...
#all_samp <- readRDS(paste0(file_path,continent,"PollenSamples.RDS"))
...
#samp_harmonized[samp_harmonized$datasetid == 52590 & samp_harmonized$sampleid == 525745,]

length(unique(all_samp$datasetid))
...
nrow(samp_harmonized %>% dplyr::filter(variablename %in% taxa))
...
samp_prop[is.na(samp_prop$prop),]

# Changes Latin names to common names & reduces dataset to info needed for mapping 
final <- samp_binned %>%
  mutate(taxa = factor(x = variablename, taxa, common_name)) %>%
  group_by(time, lat, long, samples, taxa) %>%
  select(time, lat, long, samples, taxa) 
  
final$samples[is.na(final$samples)] <- 0
