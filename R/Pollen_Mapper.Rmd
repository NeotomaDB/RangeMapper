...
Later on, rCartoAPI pushes the data you've downloaded to Carto. It depends on httr and devtools, but you won't reference those libraries directly.  
...
With this set up complete, you'll be ready to extract your first dataset. 

First, replace all instances of "taxon" with the taxon you'd like to explore. Be aware that these values are case-sensitive. It might be helpful to browse Neotoma at this point, just to make sure that what you're looking for exists in its database.  
...
It's also possible to select data from a geopolitical region. That code is recorded in lines 114-116. Pick whichever of these methods work best for you. 
...
The last step creates a list of all the taxon dataset numbers.

By running this chunk several times in a row with different Taxon values (but keeping other parameters constant!), it is possible to generate lists for many taxa. All of these lists will be written together as one file in the following section.

```{r data_query}

#taxon_datasets <- get_dataset(taxonname = 'Taxon*',
                             # loc = c(set location here), 
                             #  ageyoung = set age, 
                             #  ageold = set age)
...
The first line in this dataset prepares the empty list you'll work with later. Nothing to add to this line!

The second line combines lists of different taxa dataset numbers. Here again, you should substitute "taxon" with whatever taxa you've pulled thus far. Add comma-separated arguments to this vector for each taxon you've referenced previously.
...
In this next section, you'll be working with data you've already downloaded from Neotoma and stored in a local file. Now, you'll retrieve that data via the proper file path and compile it. Note that data_downloads may be a generic name, if you've chosen to call your file something more specific.
...
This chuck fetches total counts for all taxa observations at every location and linerally interpolates all the data into bins of 500 years. 

In lines 164 and 167, replace every instance of Taxon with the specific names of the taxa you've examined, being mindful that taxon names are case-sensitive in this code. Run additional itterations of lines 164 and 167 for each taxon you've observed.
...
#interp_dl <- data.frame(comp_dl[,1:10],
  ...
  #summarize(taxon = mean(taxon) * 100)
...
```{r filter}
#timefltr_output <- dplyr::filter (interp_dl, time >= min year, time < max year)
